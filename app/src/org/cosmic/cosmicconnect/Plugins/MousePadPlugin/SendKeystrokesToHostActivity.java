...
import org.cosmic.cosmicconnect.Helpers.SafeTextChecker;
...
import org.cosmic.cosmicconnect.databinding.ActivitySendkeystrokesBinding;
...
public class SendKeystrokesToHostActivity extends BaseActivity<ActivitySendkeystrokesBinding> {
    ...
    protected void onStart() {
        ...
        if (!prefs.getBoolean(getString(R.string.pref_sendkeystrokes_enabled), true)) {
            ...
            if ("text/x-keystrokes".equals(type)) {
                ...
                if (contentIsOkay) {
                    List<Device> reachableDevices = CosmicConnect.getInstance().getDevices().values().stream()
                ...
                CosmicConnect.getInstance().addDeviceListChangedCallback("SendKeystrokesToHostActivity", () -> runOnUiThread(this::updateDeviceList));
    ...
    protected void onStop() {
        CosmicConnect.getInstance().removeDeviceListChangedCallback("SendKeystrokesToHostActivity");
        super.onStop();
    }
    ...
    private void sendKeys(Device deviceId) {
        ...
        if (getBinding().textToSend.getText() != null && (toSend = getBinding().textToSend.getText().toString().trim()).length() > 0) {
            MousePadPlugin plugin = CosmicConnect.getInstance().getDevicePlugin(deviceId.getDeviceId(), MousePadPlugin.class);
            if (plugin == null) {
                finish();
                return;
            }
