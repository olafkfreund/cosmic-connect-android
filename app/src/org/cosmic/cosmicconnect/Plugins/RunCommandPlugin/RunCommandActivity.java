...
import org.cosmic.cosmicconnect.databinding.ActivityRunCommandBinding;
...
public class RunCommandActivity extends BaseActivity<ActivityRunCommandBinding> {
    ...
    private void updateView() {
        RunCommandPlugin plugin = CosmicConnect.getInstance().getDevicePlugin(deviceId, RunCommandPlugin.class);
        if (plugin == null) {
            finish();
            return;
        }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Device device = CosmicConnect.getInstance().getDevice(deviceId);
        if (device != null) {
    ...
    public boolean onContextItemSelected(MenuItem item) {
        ...
        if (item.getItemId() == R.id.copy_url_to_clipboard) {
            ...
            String url = "cosmicconnect://runcommand/" + deviceId + "/" + entry.getKey();
