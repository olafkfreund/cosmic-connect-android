...
class PointerAccelerationProfileFactory {
    ...
    private abstract class SpeedBasedAccelerationProfile : PointerAccelerationProfile() {
        ...
        private fun addHistory(deltaX: Float, deltaY: Float, eventTime: Long) {
        ...
        override fun touchMoved(deltaX: Float, deltaY: Float, eventTime: Long) {
            ...
            val finalMultiplier = if (multiplier < 0.01f) 0.01f else multiplier

            accumulatedX += deltaX * finalMultiplier
            accumulatedY += deltaY * finalMultiplier
    ...
    private class PowerProfile : SpeedBasedAccelerationProfile() {
        override fun calculateMultiplier(speed: Float): Float {
            return 2.0f.pow(speed / 1000) - 1
        }
    }

    companion object {
        @JvmStatic
        fun getProfileWithName(name: String): PointerAccelerationProfile {
            return when (name) {
