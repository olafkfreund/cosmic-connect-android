...
import org.cosmicext.connect.Core.Payload
...
internal object AlbumArtCache {
    ...
    @JvmStatic
    val ALLOWED_SCHEMES = listOf("http", "https", "file", "cosmicextconnect")

    /**
     * A list of art url schemes that require a fetch from remote side.
     */
    @JvmStatic
    private val REMOTE_FETCH_SCHEMES = listOf("file", "cosmicextconnect")
    ...
    fun initializeDiskCache(context: Context) {
        ...
        try {
            ...
            Log.e("COSMIC/Mpris/AlbumArtCache", "Could not open the album art disk cache!", e)
        }
    ...
    fun getAlbumArt(albumUrl: String?, plugin: MprisPlugin, player: String?): Bitmap? {
        ...
        //We currently only support http(s), file, and cosmicconnect urls
        ...
        if (!this::diskCache.isInitialized) {
            Log.e("COSMIC/Mpris/AlbumArtCache", "The disk cache is not initialized!")
            return null
        }
        try {
            ...
            if (item != null) {
                ...
                if (result != null) {
                    ...
                    Log.d("COSMIC/Mpris/AlbumArtCache", "Invalid image: $albumUrl")
                }
        ...
        if (url.scheme in REMOTE_FETCH_SCHEMES) {
            //Special-case file or cosmicconnect, since we need to fetch it from the remote
    ...
    fun payloadToDiskCache(albumUrl: String, payload: Payload?) {
        ...
        if (url.scheme !in REMOTE_FETCH_SCHEMES) {
            ...
            Log.e("COSMIC/Mpris/AlbumArtCache", "Got invalid art url with payload: $albumUrl")
            payload.close()
            return
        }
        ...
        try {
            ...
            Log.e("COSMIC/Mpris/AlbumArtCache", "Disk cache problem!", e)
            payload.close()
            return
        }
    ...
    private fun fetchURL(url: Uri, payload: Payload?, cacheItem: DiskLruCache.Editor) {
        try {
            ...
            try {
                ...
                Log.e("COSMIC/Mpris/AlbumArtCache", "Problem with the disk cache", e)
            }
