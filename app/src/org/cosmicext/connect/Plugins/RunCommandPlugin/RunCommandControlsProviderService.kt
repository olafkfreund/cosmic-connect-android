...
class RunCommandControlsProviderService : ControlsProviderService() {
    ...
    override fun performControlAction(controlId: String, action: ControlAction, consumer: Consumer<Int>) {
        ...
        if (action is CommandAction) {
            ...
            if (commandEntry != null) {
                ...
                val plugin = CosmicExtConnect.getInstance().getDevicePlugin(deviceId ,RunCommandPlugin::class.java)
                if (plugin != null) {
    ...
    private fun getAllCommandsList(): List<CommandEntryWithDevice> {
        ...
        for (device in CosmicExtConnect.getInstance().devices.values) {
    ...
    private fun getCommandByControlId(controlId: String): CommandEntryWithDevice? {
        ...
        val device = CosmicExtConnect.getInstance().getDevice(controlIdParts[0])
