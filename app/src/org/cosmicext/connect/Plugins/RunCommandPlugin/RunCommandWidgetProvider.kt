...
class RunCommandWidgetProvider : AppWidgetProvider() {
    ...
    override fun onEnabled(context: Context) {
        ...
        CosmicExtConnect.getInstance().addDeviceListChangedCallback("RunCommandWidget") {
            forceRefreshWidgets(context)
        }
    }

    override fun onDisabled(context: Context) {
        CosmicExtConnect.getInstance().removeDeviceListChangedCallback("RunCommandWidget")
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (intent.action == RUN_COMMAND_ACTION) {
            ...
            val plugin = CosmicExtConnect.getInstance().getDevicePlugin(targetDevice, RunCommandPlugin::class.java)
...
internal fun updateAppWidget(
    ...
    val device: Device? = if (deviceId != null) CosmicExtConnect.getInstance().getDevice(deviceId) else null
    ...
    if (device == null) {
        ...
        // 2. the string id does not match any devices in CosmicExtConnect.getInstance()
