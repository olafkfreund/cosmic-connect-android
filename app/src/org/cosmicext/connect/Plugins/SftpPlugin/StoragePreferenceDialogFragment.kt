...
import org.cosmicext.connect.Helpers.StorageHelper
import org.cosmicext.connect.Plugins.SftpPlugin.SftpPlugin.StorageInfo.Companion.fromJSON
...
class StoragePreferenceDialogFragment : PreferenceDialogFragmentCompat(), TextWatcher {
    ...
    private val documentTreeLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
        if (result.resultCode == Activity.RESULT_OK) {
            val data = result.data ?: return@registerForActivityResult
            val uri = data.data
            takeFlags = data.flags and (Intent.FLAG_GRANT_READ_URI_PERMISSION or Intent.FLAG_GRANT_WRITE_URI_PERMISSION)

            if (uri == null) {
                return@registerForActivityResult
            }

            val callbackResult = callback!!.isUriAllowed(uri)

            if (callbackResult.isAllowed) {
