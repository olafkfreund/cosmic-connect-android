...
class PairingHandlerTest {
    private lateinit var certA: Certificate
    private lateinit var certB: Certificate
    private lateinit var sslHelper: SslHelper

    @Before
    fun setUp() {
        val context = mockk<Context>(relaxed = true)
        val rsaHelper = mockk<RsaHelper>(relaxed = true)
        sslHelper = SslHelper(context, rsaHelper)

        certA = sslHelper.parseCertificate(Base64.getMimeDecoder().decode(
        ...
        certB = sslHelper.parseCertificate(Base64.getMimeDecoder().decode(
