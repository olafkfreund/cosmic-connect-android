EXPORTS_POT_DIR=1
FILE_PREFIX=cosmicconnect-android-store

function export_pot_dir # First parameter will be the path of the directory where we have to store the pot files
{
    potdir=$1
    txt2po --no-segmentation --progress=names -P -i ./metadata/android/en-US/short_description.txt -o $potdir/cosmicconnect-android-store-short.pot
    txt2po --no-segmentation --progress=names -P -i ./metadata/android/en-US/full_description.txt -o $potdir/cosmicconnect-android-store-full.pot
}
...
function import_po_dirs # First parameter will be a path that will be a directory to the dirs for each lang and then all the .po files inside
{
    ...
    for lang in $(ls $podir); do
        if [ -f $podir/$lang/cosmicconnect-android-store-short.po -a -f $podir/$lang/cosmicconnect-android-store-full.po ]; then
            ...
            po2txt --fuzzy --progress=names -i $podir/$lang/cosmicconnect-android-store-short.po -o ./metadata/android/$lang/short_description.txt
            po2txt --fuzzy --progress=names -i $podir/$lang/cosmicconnect-android-store-full.po -o ./metadata/android/$lang/full_description.txt
        fi
    done
}
