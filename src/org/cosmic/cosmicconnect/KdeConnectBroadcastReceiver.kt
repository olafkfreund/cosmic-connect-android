...
class CosmicConnectBroadcastReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        // Log.e("CosmicConnect", "Broadcast event: "+intent.getAction());

        val action = intent.action

        when (action) {
            Intent.ACTION_MY_PACKAGE_REPLACED -> {
                Log.i("CosmicConnect", "MyUpdateReceiver")
                BackgroundService.Start(context)
            }

            Intent.ACTION_BOOT_COMPLETED -> {
                Log.i("CosmicConnect", "CosmicConnectBroadcastReceiver")
                try {
                    BackgroundService.Start(context)
            ...
            WifiManager.SUPPLICANT_CONNECTION_CHANGE_ACTION, WifiManager.WIFI_STATE_CHANGED_ACTION, ConnectivityManager.CONNECTIVITY_ACTION -> {
                Log.i("CosmicConnect", "Connection state changed, trying to connect")
