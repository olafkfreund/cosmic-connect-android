...
class RunCommandPlugin : Plugin() {
    ...
    override fun onPacketReceived(legacyNp: LegacyNetworkPacket): Boolean {
        ...
        if (np.isRunCommandList) {
            handleCommandListPacket(legacyNp)
            return true
        }
    ...
    private fun handleCommandListPacket(legacyNp: LegacyNetworkPacket) {
        _commandList.clear()
        _commandItems.clear()

        try {
            val commandListJson = legacyNp.getString("commandList") ?: return
        ...
        canAddCommand = legacyNp.getBoolean("canAddCommand", false)
    }
