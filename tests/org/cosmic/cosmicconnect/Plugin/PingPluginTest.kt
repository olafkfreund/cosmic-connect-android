package org.cosmic.cosmicconnect.Plugin
...
class PingPluginTest {
    ...
    fun startPlugin() {
        executeWithMocks { device, plugin, notificationManager ->
            ...
            verify(exactly = 1) { device.sendPacket(match { np -> np.type == "cosmicconnect.ping" && np.payload == null }) }
        }
    }
    ...
    fun wrongPacketType() {
        executeWithMocks { device, plugin, notificationManager ->
            val np = NetworkPacket("cosmicconnect.wrench")
    ...
    fun sendsNotificationNoMessage() {
        executeWithMocks { device, plugin, notificationManager ->
            val np = NetworkPacket("cosmicconnect.ping")
